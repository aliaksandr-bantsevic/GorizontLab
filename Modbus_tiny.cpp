...
WORD crc16(uint8_t *buffer, WORD buffer_length)
{
    ...
    while (buffer_length--) {
		...
		crc_hi = crc_lo ^ rtu_table_crc_hi[i];
		crc_lo = rtu_table_crc_lo[i];
    }
...
int request_modbus_rtu (modbus_rtu_cmd cmd, BYTE* buf, int* len)
{
	...
	*len = idx;
	...
	BYTE bbb [] = {0x07, 0x04, 0x00, 0x00, 0x00, 0x04, 0xf1, 0xaf};
    memcpy(buf,bbb,8);
